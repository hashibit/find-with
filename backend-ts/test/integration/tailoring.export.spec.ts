...
/** Create a resume row with no bullets yet. */
async function createResume(): Promise<TailoringResume> {
...
/** Insert bullet rows for a resume in the given statuses. */
async function addBullets(
  resumeId: string,
  statuses: Array<{ status: BulletStatus; id?: string }>,
): Promise<TailoringBullet[]> {
  const entities = statuses.map(({ status, id }, pos) =>
    bulletRepo.create({
      id: id ?? ulid(),
      resumeId,
      sectionTitle: 'Work Experience',
      position: pos,
      text: `Bullet text for ${id ?? pos}`,
      source: 'MATERIAL',
      sourceId: null,
      status,
    }),
  );
  return bulletRepo.save(entities);
}
...
describe('TailoringService.exportResume — integration', () => {
  describe('PENDING bullet guard', () => {
    ...
    it('includes the pending bullet IDs in the error response', async () => {
      ...
      await addBullets(resume.id, [
        ...
        { status: BulletStatus.PENDING, id: pendingId },
      ]);
    ...
    it('does not consume quota when the guard fires', async () => {
      const resume = await createResume();
      await addBullets(resume.id, [{ status: BulletStatus.PENDING }]);
  ...
  describe('successful export', () => {
    it('returns plain-text content built from section bullets', async () => {
      ...
      await addBullets(resume.id, [
        { status: BulletStatus.CONFIRMED },
        { status: BulletStatus.USER_EDITED },
      ]);
    ...
    it('consumes quota on export', async () => {
      const resume = await createResume();
      await addBullets(resume.id, [{ status: BulletStatus.CONFIRMED }]);
