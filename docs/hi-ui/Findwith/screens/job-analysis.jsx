  ...
  return (
    ...
    >
      ...
      <div className="qcard-b" style={{ padding: '10px 12px', fontSize: 12.5, lineHeight: 1.6 }}>
        <div>
          <span className="muted">在做什么 · </span>支付基础设施，B2B / 开发者产品
        </div>
        <div>
          <span className="muted">规模 · </span>~8,400 人 · 增长放缓但未大规模裁员
        </div>
        <div>
          <span className="muted">Glassdoor · </span>4.1 ⭐ · 面试评价"流程长但人专业"
        </div>
        <div
          style={{
            marginTop: 6,
...
function JDCard({ tweaks, activeIdx }) {
  const items = [
    { txt: <>5+ 年 PM 经验，B2B / 开发者产品优先</> },
    {
      txt: (
        <>
          强调 <strong>cross-functional leadership</strong>
        </>
      ),
    },
      ...
      <div className="qcard-b" style={{ padding: '10px 12px', fontSize: 12.5, lineHeight: 1.7 }}>
        {items.map((it, i) => {
          ...
          return (
            ...
            >
              <span style={{ width: 14, color: isActive ? 'var(--accent)' : 'var(--mute)' }}>
                ·
              </span>
              <span style={{ flex: 1, color: isActive ? 'var(--ink)' : 'var(--ink-2)' }}>
                {it.txt}
              </span>
              {isActive && (
  ...
  return (
    <div className="qcard" style={{ marginTop: 8 }}>
      <div className="qcard-b" style={{ padding: '12px 14px' }}>
        ...
        >
          <div className="muted lab" style={{ marginBottom: 6 }}>
            关键缺口{' '}
            <span
              style={{
                color: 'var(--mute-2)',
          ...
          {[
            ...
            return (
              ...
              >
                ...
                <span style={{ flex: 1 }}>
                  <strong>{g.name}</strong> <span className="muted">· {g.tail}</span>
                </span>
                {active && (
                  <span style={{ fontSize: 10, color: 'var(--accent)', whiteSpace: 'nowrap' }}>
                    挖掘 →
  ...
  return (
    <Q qStyle={tweaks.qStyle} qColor={tweaks.aColor}>
      <div style={{ fontSize: 13 }}>
        我的建议：<strong>值得投</strong>。但简历需要重点改两处。
      </div>
      <div style={{ marginTop: 10, display: 'flex', gap: 6, flexWrap: 'wrap' }}>
        <button className="btn primary" style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
          想投，开始定制
          <svg width="11" height="11" viewBox="0 0 12 12" style={{ marginLeft: 2 }}>
            <path
              d="M3 9l6-6M4 3h5v5"
              stroke="currentColor"
              strokeWidth="1.4"
      ...
      <div
        className="muted"
        style={{ fontSize: 10.5, marginTop: 8, display: 'flex', alignItems: 'center', gap: 4 }}
      >
  ...
  return (
    <Frame variant="a">
      ...
      >
        <Sys>已接入岗位 · 02:14 PM</Sys>
        <Q qStyle={tweaks.qStyle} qColor={tweaks.aColor}>
          <CompanyCard tweaks={tweaks} active />
        </Q>
        ...
        >
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            <SourceRow
              icon="📰"
              label="TechCrunch · 近 30 天"
              badge="3 篇"
              snippet="估值 $65B 持平上轮 · 削减硬件投入，聚焦 platform 收入"
            />
            <SourceRow
              icon="🪟"
              label="Glassdoor 摘要 · 47 条"
              badge="4.1 ★"
              snippet="高频词：'流程长 (32%)' · '人专业 (41%)' · '薪资稳健 (28%)'"
            />
            <SourceRow
              icon="✉️"
              label="公司博客 + 招聘页"
              badge="5 篇"
              snippet="近期主推：global expansion (LATAM)、AI 工具集 · 减少强调 crypto"
            />
          </div>
          ...
          >
            <div className="lab muted" style={{ marginBottom: 5 }}>
              Quinn 解读
            </div>
            <div>
              这个岗位上线在 <strong>削减硬件 + push platform 收入</strong> 后两周——他们要的人是能
              <strong>把开发者产品商业化</strong>的，不是再做工具。所以你简历里 Notion 那段 billing
              经验，是 hidden gem。
            </div>
          </div>
        </div>
  ...
  return (
    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 3 }}>
        ...
        <span style={{ flex: 1 }}></span>
        <span className="chip soft" style={{ fontSize: 9.5, padding: '1px 5px' }}>
          {badge}
        </span>
      </div>
      <div className="muted" style={{ lineHeight: 1.5, paddingLeft: 18 }}>
        {snippet}
      </div>
    </div>
  );
      ...
      >
        ...
        <Q qStyle={tweaks.qStyle} qColor={tweaks.aColor}>
          <div style={{ fontSize: 12.5, lineHeight: 1.55 }}>
            "Cross-functional leadership" — 翻译过来他们要的是
            <strong>能不靠权威撬动多个团队</strong>。
          </div>
          <div className="qcard" style={{ marginTop: 8 }}>
            <div className="qcard-h">
              <span>你的对应素材</span>
            ...
            <div style={{ padding: '4px 0' }}>
              ...
              <MatRow
                id={21}
                txt="oncall 诊断工具被 4 个团队复用，事故 -50%"
                company="Notion"
                rel={78}
              />
              <MatRow
                id={16}
                txt="独立带 2 个 intern 完成 ship-it"
                company="Mercury"
                rel={61}
                dim
              />
...
function MatRow({ id, txt, company, rel, primary, dim }) {
  return (
    ...
    >
      <span className="muted tnum" style={{ fontSize: 10.5, marginTop: 1, minWidth: 22 }}>
        #{id}
      </span>
  ...
  return (
    <Frame variant="a">
      <SidePanel
        ...
        jobContext={JOB_CTX}
        inputDim={false}
        inputPlaceholder="例：上次产品改版前，数据分析告诉我..."
      >
        <Q qStyle={tweaks.qStyle} qColor={tweaks.aColor}>
          <div style={{ fontSize: 13 }}>三层匹配度——</div>
        ...
        <Q qStyle={tweaks.qStyle} qColor={tweaks.aColor}>
          ...
          <div className="qcard" style={{ marginTop: 8 }}>
            ...
            >
              <span>下一个：</span>
              <span style={{ padding: '2px 6px', borderRadius: 4, background: 'var(--bg-sunk)' }}>
                反直觉的发现
              </span>
              <span style={{ padding: '2px 6px', borderRadius: 4, background: 'var(--bg-sunk)' }}>
                验证过程
