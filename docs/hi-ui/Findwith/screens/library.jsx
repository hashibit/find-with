...
const MATERIALS = [
  {
    id: 23,
    txt: '入职 60 天内主导团队 dev workflow 重构，将 PR-to-prod 的中位时间从 4.2 天压到 2.6 天，为 32 名工程师减负',
    tag: '主动性',
    date: '04/12',
    conf: true,
    expanded: true,
    raw: '我在新公司前两个月就梳理了团队的开发流程',
    src: '2026-04-12 对话',
  ...
  {
    id: 21,
    txt: '识别重复 oncall 模式，独立 ship 诊断工具，-50% incident',
    tag: '主动性',
    date: '04/08',
    conf: true,
  },
  {
    id: 20,
    txt: '撰写并主导 RFC，推动支付服务从 MySQL 迁移至 Postgres',
    tag: '技术深度',
    date: '03/30',
    conf: true,
  },
  {
    id: 19,
    txt: '第一个月独立完成 Billing webhook 重写',
    tag: '技术深度',
    date: '03/22',
    conf: true,
  },
  ...
  {
    id: 16,
    txt: '独立带 2 个 intern，完成各自的 ship-it 项目',
    tag: 'Mentor',
    date: '02/28',
    conf: true,
  },
      ...
      <div style={{ display: 'flex', gap: 0, padding: '4px 14px 0' }}>
        {['基本', '工作', '项目', '技能', '素材库'].map((t, i) => {
          ...
          return (
            <div
              key={t}
              className={isB && active ? 'serif' : ''}
              style={{
                padding: '10px 10px',
                fontSize: 12,
                fontWeight: active ? (isB ? 500 : 600) : 400,
                color: active ? 'var(--ink)' : 'var(--mute)',
                borderBottom: active
                  ? `2px solid ${isB ? 'var(--accent)' : 'var(--ink)'}`
            ...
            >
              ...
              {active && (
                <span
                  className={`muted tnum${isB ? ' italic' : ''}`}
                      ...
                      .reduce(
                        (a, b, i) =>
                          i
                            ? [
                                ...a,
                                <span key={'s' + i} className="muted" style={{ fontSize: 10 }}>
                                  ·
                                </span>,
                                b,
                              ]
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'baseline', gap: 6, minWidth: 0 }}>
                    ...
                    <span
                      style={{
                        fontSize: 10,
                        color: TAG_COLORS[m.tag] || 'var(--mute)',
                        flexShrink: 0,
                        fontWeight: 500,
                      }}
                    >
    ...
    <Frame variant="b">
      <SidePanel variant="b" tab="profile" qStyle={tweaks.qStyle} qColor={tweaks.bColor} hideInput>
        ...
        >
          ...
          >
            {I.search}
            <span className="italic serif">搜索 · 标签 · 时间</span>
          </div>
          <button className="btn" style={{ padding: '5px 9px', fontSize: 11, borderRadius: 4 }}>
            {I.plus}
          </button>
        </div>
              ...
              m.expanded ? (
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'baseline', gap: 6 }}>
                    ...
                    <span
                      className="serif"
                      style={{ color: 'var(--ink)', flex: 1, fontSize: 12.5, lineHeight: 1.55 }}
                    >
                      {m.txt}
                    </span>
                    <span className="muted tnum" style={{ fontSize: 10.5 }}>
                  ...
                  >
                    ...
                    <span
                      ...
                      style={{ fontSize: 10, fontFamily: 'Source Serif 4, serif' }}
                    >
                      {m.src} · 用过 {m.uses} 次
                    </span>
                  </div>
