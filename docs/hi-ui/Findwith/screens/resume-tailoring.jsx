        ...
        >
          <button
            style={{
              ...
              border: 'none',
              color: isB ? 'var(--b-mute)' : 'var(--a-mute)',
              cursor: 'pointer',
              display: 'flex',
              alignItems: 'center',
              gap: 6,
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 4 }}>
                <span
                  style={{
                    fontSize: 13,
                    color: isB ? 'var(--b-mute-2)' : 'var(--a-mute-2)',
                    textDecoration: 'line-through',
                  }}
                >
                  62
                </span>
                <span style={{ color: isB ? 'var(--b-mute-2)' : 'var(--a-mute-2)' }}>
                  {I.arrowR}
                </span>
                <span
                  className={isB ? 'serif' : ''}
          ...
          <button className="btn">
            {I.doc}
            <span style={{ marginLeft: 4 }}>纯文本</span>
          </button>
          <button
            className="btn primary"
            style={{
              background: isB ? bColor : '#09090B',
              borderColor: isB ? bColor : '#09090B',
              color: '#fff',
            }}
          >
            导出 PDF
        ...
        <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
          ...
          <div
            style={{
              ...
              overflow: 'hidden',
              borderRight: `1px solid ${isB ? 'var(--b-line)' : 'var(--a-line)'}`,
            }}
          >
  ...
  return (
    <div
      style={{
        marginBottom: 18,
        paddingBottom: 12,
        borderBottom: `1px solid ${isB ? 'var(--b-line-2)' : 'var(--a-line-2)'}`,
      }}
    >
      <div
        style={{
      ...
      <div
        className={isB ? 'serif italic' : ''}
        style={{ fontSize: 12, color: isB ? 'var(--b-mute)' : 'var(--a-mute)' }}
      >
        {sub}
      </div>
    </div>
...
function ResumeBody({ variant, mode, accent }) {
  ...
  const Header = (
    <div>
      <div
        className={isB ? 'serif' : ''}
        style={{ fontSize: 22, fontWeight: 600, letterSpacing: '-0.02em', color: ink }}
      >
        Marcus Chen
      </div>
      ...
      <div style={{ fontSize: 12, lineHeight: 1.55, color: ink, opacity: 0.78 }}>
        ...
        <div style={{ marginTop: 18 }}>
          ...
          <div style={{ marginTop: 14 }}>
            ...
            {ORIGINAL_BULLETS.map((b) => (
              <Bullet key={b.id} variant={variant} text={b.text} />
            ))}
          </div>
        </div>
      </div>
  ...
  return (
    <div
      style={{
        ...
        paddingBottom: 6,
        borderBottom: `1px solid ${isB ? 'var(--b-ink)' : 'var(--a-ink)'}`,
        color: isB ? 'var(--b-ink)' : 'var(--a-ink)',
      }}
    >
      {children}
  ...
  return (
    ...
    >
      <div className={isB ? 'serif' : ''} style={{ fontSize: 13, fontWeight: 600 }}>
        {co}{' '}
        <span style={{ fontWeight: 400, color: isB ? 'var(--b-mute)' : 'var(--a-mute)' }}>
          · {role}
        </span>
      </div>
      <div
...
function Bullet({ variant, text }) {
  return (
    <div style={{ display: 'flex', gap: 8, marginTop: 5, fontSize: 12, lineHeight: 1.5 }}>
      <span
        style={{ color: variant === 'b' ? 'var(--b-mute-2)' : 'var(--a-mute-2)', flexShrink: 0 }}
      >
        —
      </span>
      <span>{text}</span>
    ...
    <>
      ...
      >
        ...
        <div style={{ flex: 1 }}>
          ...
          <div
            style={{ fontSize: 11, color: isB ? 'var(--b-mute)' : 'var(--a-mute)', marginTop: 1 }}
          >
            4 处建议 · 2 处缺口 · 1 处待确认
          </div>
        </div>
      ...
      >
        ...
        >
          ...
          <div
            style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 6, fontSize: 11 }}
          >
        ...
        >
          <div className="lab" style={{ color: accent, marginBottom: 6 }}>
            缺口挖掘 · 1/2
          </div>
          <div
            className={isB ? 'serif' : ''}
            style={{ fontSize: 13, lineHeight: 1.55, color: isB ? 'var(--b-ink)' : 'var(--a-ink)' }}
          >
          ...
          <div
            style={{
              ...
              marginTop: 6,
              lineHeight: 1.55,
              color: isB ? 'var(--b-ink-2)' : 'var(--a-ink-2)',
            }}
          >
        ...
        <div>
          <div
            className="lab"
            style={{ color: isB ? 'var(--b-mute)' : 'var(--a-mute)', marginBottom: 8 }}
          >
            本次定制做了什么
          </div>
          <div
        ...
        >
          ...
          <span
            style={{ flex: 1, fontSize: 12, color: isB ? 'var(--b-mute-2)' : 'var(--a-mute-2)' }}
          >
      ...
      >
        ...
        <span
          style={{
            fontSize: 18,
            fontWeight: isB ? 500 : 600,
            color: highlight ? accent : isB ? 'var(--b-ink)' : 'var(--a-ink)',
          }}
        >
          {now}
        </span>
