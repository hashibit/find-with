"use strict";
(() => {
  // src/content-scripts/shared/dom.ts
  ...
  // src/content-scripts/shared/sanitize.ts
  function sanitizeText(input) {
    if (!input) return "";
    const stripped = purify.sanitize(input, { ALLOWED_TAGS: [], ALLOWED_ATTR: [] });
    return stripped.replace(/\s+/g, " ").trim();
  }

  // src/content-scripts/linkedin/job-detail.ts
    ...
    btn.addEventListener("click", async () => {
      ...
      try {
        ...
        if (result?.error) {
          ...
          const jobId = result?.capture?.id ?? result?.radarItem?.id;
