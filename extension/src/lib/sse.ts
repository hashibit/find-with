/**
 * SSE implementation using fetch + ReadableStream.
 * Shared between background service worker and sidepanel dev mode.
    ...
    try {
      while (true) {
        ...
        while ((idx = buf.indexOf('\n\n')) >= 0) {
          ...
          if (ev) {
            if (ev.id && persistEventId) {
              persistEventId(ev.id);
            }
            onEvent(ev);
          }
        }
